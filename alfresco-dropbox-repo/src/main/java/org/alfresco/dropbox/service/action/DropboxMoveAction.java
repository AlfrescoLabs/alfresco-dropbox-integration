...
{
    private final static Log   logger            = LogFactory.getLog(DropboxMoveAction.class);
        ...
        for (final Map.Entry<String, NodeRef> syncedUser : syncedUsers.entrySet())
        {
            AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Metadata>()
            {
                    ...
                    throws Exception
                {
                    ...
                    dropboxService.persistMetdata(metadata, ((ChildAssociationRef)action.getParameterValue(DROPBOX_TO_PATH)).getChildRef());
